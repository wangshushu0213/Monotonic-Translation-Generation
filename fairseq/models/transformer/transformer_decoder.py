...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        # encoder_attn_mask = self.get_attention_mask(x, enc.size(0))################################################################ added by wss 推理注释行2
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _ = layer(
                ...
                # encoder_attn_mask=encoder_attn_mask,################################################################ added by wss 推理注释行3
            )
    ...
    def get_attention_mask(self, x, src_len):        ######################################### added by wss
        # if waitk is None:
        #     if self.multi_waitk:
