...
class FairseqTask(object):
    ...
    def build_generator(
        ...
        from fairseq.monotonic_sequence_generator import (
            MonotonicSequenceGenerator,
        )
        ...
        if seq_gen_cls is None:
            if getattr(args, "print_alignment", False):
                ...
                generator_cls = getattr(args, "generator_class")
                if generator_cls == "standard":
                    seq_gen_cls = SequenceGenerator
                elif generator_cls == "monotonic":
                    seq_gen_cls = MonotonicSequenceGenerator
                    for key in ("latency", "initial_beam_size", "encode_prefix_once",
                                "num_bos_tokens", "num_eos_tokens", "disable_rescoring"):
                        extra_gen_cls_kwargs[key] = getattr(args, key)
                else:
                    raise Exception("Unsupported sequence generator: " + generator_cls)
