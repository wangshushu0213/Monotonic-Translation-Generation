...
class GenerationConfig(FairseqDataclass):
    ...
    generator_class: Optional[str] = field(
        default="standard",
        metadata={"help": "choose the sequence generator from standard (SequenceGenerator, default) and "
                          "monotonic (MonotonicSequenceGenerator)"},
    )
    latency: int = field(
        default=5,
        metadata={"help": "the wait-k latency (difference between the length of observed source and generated "
                          "target)"},
    )
    initial_beam_size: int = field(
        default=10,
        metadata={"help": "the initial beam size for generating candidates for rescoring"},
    )
    encode_prefix_once: bool = field(
        default=False,
        metadata={"help": "if set, represent prefix-specific encoding using full-sentence encoding of source "
                          "prefixes"},
    )
    num_bos_tokens: int = field(
        default=2,
        metadata={"help": "number of extra tokens before the actual source sentence (excluding padding)"},
    )
    num_eos_tokens: int = field(
        default=1,
        metadata={"help": "number of extra tokens after the actual source sentence (excluding padding)"},
    )
    disable_rescoring: bool = field(
        default=False,
        metadata={"help": "if set, disable full sentence rescoring"}
    )
